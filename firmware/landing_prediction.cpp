...
#define GPS_HDOP_MAX   5.0f        /* HDOP threshold for valid fix         */
...
/* horizontal */
static float  s_lat_prev       = 0.0f;
static float  s_lon_prev       = 0.0f;
static float  s_v_north_filt   = 0.0f;
static float  s_v_east_filt    = 0.0f;

/* last valid GPS fix (for fallback) */
static float  s_last_valid_lat = 0.0f;
static float  s_last_valid_lon = 0.0f;
...
LandingPrediction computeLandingPrediction(SensorData* s) {
    ...
    if (s_first_call) {
        ...
        result.pred_lat      = s->gps_lat;
        result.pred_lon      = s->gps_lon;
        result.uncertainty_m = UNC_MAX;
        result.t_to_land_s   = TTL_MAX;
        result.gps_fallback  = (s->fix_type < 2 || s->hdop > GPS_HDOP_MAX) ? 1 : 0;
        return result;
    }
    ...
    int gps_fallback = 0;
    bool gps_valid = (s->fix_type >= 2) && (s->hdop <= GPS_HDOP_MAX);

    if (gps_valid) {
        /* PRIMARY: compute velocity from successive GPS fixes */
        float v_north = (s->gps_lat - s_lat_prev) * DEG_TO_M / dt;
        float v_east  = (s->gps_lon - s_lon_prev) * DEG_TO_M
                        * cosf(s->gps_lat * PI / 180.0f) / dt;
    ...
    float origin_lat = gps_fallback ? s_last_valid_lat : s->gps_lat;
    float origin_lon = gps_fallback ? s_last_valid_lon : s->gps_lon;
